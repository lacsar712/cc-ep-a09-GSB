<template>
  <div class="page">
    <div>
      ...
      <p class="muted" style="margin-top: 0">按指标名查找所有记录过该指标的 Run</p>
    </div>
    ...
    <div v-if="searched" class="card">
      <n-data-table :columns="columns" :data="rows" :loading="loading" :bordered="false">
        <template #empty>未找到记录过「{{ lastQuery }}」指标的 Run</template>
      </n-data-table>
    </div>
  </div>
</template>
...
import { searchRunsByMetric } from '../api/client'
...
const columns = [
  ...
  { title: '最近值', key: 'latest_value' },
  { title: 'Step', key: 'latest_step', width: 80 },
  {
    title: '记录时间',
    key: 'recorded_at',
    render(row) {
...
async function search() {
  ...
  loading.value = true
  try {
    rows.value = await searchRunsByMetric(name)
